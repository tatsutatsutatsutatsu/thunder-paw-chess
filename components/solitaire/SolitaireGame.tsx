...
function PlayingCard({ card, onClick, onDoubleClick, onDragStart, selected, style }: { card: Card; onClick?: () => void; onDoubleClick?: () => void; onDragStart?: (event: React.DragEvent) => void; selected?: boolean; style?: React.CSSProperties }) {
  ...
  return <button type="button" className={`sol-card ${card.faceUp ? 'face-up' : 'face-down'} ${card.faceUp && (card.suit === 'hearts' || card.suit === 'diamonds') ? 'red' : ''} ${selected ? 'picked' : ''}`} style={style} aria-label={label} aria-pressed={card.faceUp && selected ? true : undefined} draggable={card.faceUp} onClick={onClick} onDoubleClick={onDoubleClick} onDragStart={onDragStart}>
    {card.faceUp ? <><span className="sol-card-corner">{rankMark(card.rank)}<small>{suitMark[card.suit]}</small></span><span className="sol-card-center"><img src="/electric-cat-mark.svg" alt="" /></span><span className="sol-card-suit">{suitMark[card.suit]}</span></> : <span className="sol-card-bolt"><img src="/electric-cat-mark.svg" alt="" /></span>}
  </button>;
}
...
export default function SolitaireGame() {
  ...
  function showHint() {
    ...
    if (hint.kind === 'move') {
      ...
      setMessage(`${suitMark[hint.card.suit]}${rankMark(hint.card.rank)}を${hint.target.kind === 'foundation' ? `${suitMark[suits[hint.target.index]]}の組札` : `場札${hint.target.index + 1}`}へ移動できます。`);
    } else if (hint.kind === 'seek') {
      setSelected(null);
      setMessage(`山札をめくって${suitMark[hint.card.suit]}${rankMark(hint.card.rank)}を探しましょう。山札が空ならタップして戻せます。`);
    } else {
      setSelected(null);
      setMessage(hint.kind === 'won' ? 'クリアです！' : '動かせるカードがありません。新しいゲームを始められます。');
    }
  }
  ...
  return <main className="sol-app">
    ...
    <div className="sol-intro"><div><p className="sol-eyebrow">THUNDER PAW MINI GAMES · 02</p><h1>電気猫のソリティア</h1><p>ひらめく一手で、4つの組札を完成させよう。</p></div><div className="sol-mascot"><img src="/electric-cat-mark.svg" alt="電気猫の2Dマーク" /><Zap size={28} fill="currentColor" aria-hidden="true" /></div></div>
