...
export function createChessScene(host:HTMLDivElement,onSquare:(s:Square)=>void,onFailure:()=>void){
 ...
 function resetView(){camera.position.set(flipped?.5:-.5,12.5,flipped?-13:13);camera.zoom=.86;camera.lookAt(controls.target);camera.updateProjectionMatrix();controls.update();dirty=true;}
 resetView();
