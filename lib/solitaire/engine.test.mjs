...
test('only an ace starts a foundation and only same-suit successors follow', () => {
  ...
  assert.equal(move(game, { kind: 'waste' }, { kind: 'foundation', index: 0 }), null);
  game = { ...game, waste: [card('hearts', 1)] };
  const next = move(game, { kind: 'waste' }, { kind: 'foundation', index: 0 });
  assert.equal(next.foundations[0].length, 1);
  assert.equal(move({ ...next, waste: [card('diamonds', 2)] }, { kind: 'waste' }, { kind: 'foundation', index: 0 }), null);
  assert.equal(move({ ...next, waste: [card('hearts', 2)] }, { kind: 'waste' }, { kind: 'foundation', index: 0 }).foundations[0].length, 2);
});
