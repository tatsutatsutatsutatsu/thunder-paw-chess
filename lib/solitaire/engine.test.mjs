...
import { draw, findHint, isWon, move, newGame, rescue } from './engine.ts';
...
test('hint points to a legal visible move and searches the stock before declaring a dead end', () => {
  const game = empty();
  game.tableau[0] = [card('hearts', 1)];
  assert.deepEqual(findHint(game), { kind: 'move', source: { kind: 'tableau', index: 0, cardIndex: 0 }, target: { kind: 'foundation', index: 1 }, card: card('hearts', 1) });
  const stockOnly = empty();
  stockOnly.stock = [card('clubs', 1, false)];
  assert.deepEqual(findHint(stockOnly), { kind: 'seek', card: card('clubs', 1, false) });
  assert.deepEqual(findHint(empty()), { kind: 'stuck' });
  const kingOnly = empty();
  kingOnly.tableau[0] = [card('spades', 13)];
  assert.deepEqual(findHint(kingOnly), { kind: 'stuck' });
});

test('optional rescue only works at a dead end and preserves all cards', () => {
  const blocked = empty();
  blocked.tableau[0] = [card('hearts', 8, false), card('clubs', 2)];
  assert.equal(findHint(blocked).kind, 'stuck');
  const helped = rescue(blocked);
  assert.equal(helped.tableau[0][0].faceUp, true);
  assert.deepEqual(helped.stock[0], card('clubs', 2, false));
  assert.equal(helped.tableau.flat().length + helped.stock.length, 2);
  const playable = empty();
  playable.waste = [card('hearts', 1)];
  assert.equal(rescue(playable), null);
});
